...
DEFAULTS = {
    ...
    "OfficeType":     "Помещение под офис",  # «Что сдаёте»: помещение (не «только юрадрес»)
    "Layout":         "Кабинетная",        # Планировка офиса (Кабинетная/Открытая)
...
CHOICES = {
    ...
    "OfficeType":     ["Помещение под офис", "Только юридический адрес"],
    "Layout":         ["Кабинетная", "Открытая"],
...
def _add_ad(root, o, cfg) -> bool:
    ...
    if obj == "Офисное помещение":
        ...
        T("Layout", cfg["Layout"])           # Планировка — обязательна у офиса (Помещение под офис)
