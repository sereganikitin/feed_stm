...
def enrich_commercial(lot, plan_url: str, template_url: str, template_ext: str,
    ...
    # Элемент — либо rect, либо {"rect":(...), "color":(r,g,b)} для индивидуального цвета.
    for cr in layout.get("cover_rects", []):
        if isinstance(cr, dict):
            rect, col = cr["rect"], cr.get("color", (255, 255, 255))
        else:
            rect, col = cr, layout.get("cover_color", (255, 255, 255))
        draw.rectangle(rect, fill=tuple(col) + (255,))
